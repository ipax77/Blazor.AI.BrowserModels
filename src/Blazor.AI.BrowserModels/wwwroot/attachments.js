...
export function createAttachments(root, supported, onDispose) {
    const picker = root.querySelector('[data-image-picker]');
    const fields = root.querySelector('[data-image-controls]');
    ...
    function add(files) {
        ...
        for (const file of files) {
            ...
            if (!file.size || file.size > maxBytes) {
                errors.push(`${file.name}: choose a nonempty image no larger than 10 MiB.`);
                continue;
            }
            if (entries.length === maxFiles) {
                errors.push('You can attach up to four images. Remove an image to add another.');
                break;
            }
    ...
    const observer = new MutationObserver(() => { if (!root.isConnected)
        dispose(); });
    ...
    return {
        snapshot() {
            if (disposed)
                throw new Error('The attachments have been disposed.');
            return entries.map(entry => entry.file);
        },
        clear,
        setDisabled(value) { disabled = value; update(); },
