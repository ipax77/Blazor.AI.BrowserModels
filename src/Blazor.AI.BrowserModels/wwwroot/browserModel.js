...
export function attach(button, callback, settings = {}, panel = null, enableImages = false, systemPrompt = null, systemPromptInput = null, stateless = false) {
    ...
    let nextImageId = 0;
    let imageNamespace;
    function retainImages(images) {
        if (disposed || !session)
            throw new Error('Initialize the browser model first.');
        if (!stateless)
            throw new Error('Image retention requires Stateless session mode.');
        if (generating)
            throw new Error('A reply is already being generated.');
        if (!attachments.has(images))
            throw new Error('These attachments do not belong to this session.');
        const files = images.snapshot();
        if (!files.length)
            return null;
        if (!supportsImages)
            throw new Error('Image attachments are unsupported in this session.');
    ...
    const topKInput = panel?.querySelector('input[name="topK"]');
    const temperatureInput = panel?.querySelector('input[name="temperature"]');
    const fields = panel?.querySelector('fieldset');
    const samplingMessage = panel?.querySelector('[data-sampling-status]');
    ...
    if (topKInput)
        topKInput.value = settings.topK?.toString() ?? '';
    if (temperatureInput)
    ...
    function updateControls() {
        if (fields)
            fields.disabled = disposed || initializing || !!session || samplingInfo.support !== 'Supported';
        if (systemPromptInput)
            systemPromptInput.disabled = disposed || initializing || !!session;
    }
    updateControls();
    async function discoverSampling() {
        if (!globalThis.isSecureContext || typeof model?.params !== 'function')
            return emptySamplingInfo('Unsupported');
        try {
    ...
    function samplingOptions() {
        ...
        if (topKInput?.validity.badInput || temperatureInput?.validity.badInput)
            throw new Error('Enter valid numeric sampling settings.');
        ...
        if (topK == null && temperature == null)
            return undefined;
        if (samplingInfo.support !== 'Supported')
            throw new Error('Numeric sampling support is not confirmed. Clear Top K and Temperature to use browser defaults.');
        const resolvedTopK = topK ?? samplingInfo.defaultTopK;
        const resolvedTemperature = temperature ?? samplingInfo.defaultTemperature;
        if (!positiveInteger(resolvedTopK) || resolvedTopK > samplingInfo.maxTopK)
            throw new Error(`Top K must be an integer from 1 to ${samplingInfo.maxTopK}.`);
        if (!nonnegative(resolvedTemperature) || resolvedTemperature > samplingInfo.maxTemperature)
            throw new Error(`Temperature must be a finite number from 0 to ${samplingInfo.maxTemperature}.`);
        return { topK: resolvedTopK, temperature: resolvedTemperature };
    }
    const report = (state, message) => {
        if (!disposed)
            callback.invokeMethodAsync("ReportAsync", state, message).catch(() => dispose());
    };
    async function availability() {
        if (!globalThis.isSecureContext || !model)
            return "unavailable";
    ...
    async function discoverAvailability() {
        if (enableImages && globalThis.isSecureContext && model) {
            const imageOptions = { ...options, expectedInputs: [...options.expectedInputs, { type: 'image' }] };
            try {
    ...
    function errorMessage(cause) {
        ...
        if (error.name === "NotAllowedError")
            return "Chrome could not initialize the model. Check browser permissions and click Initialize again.";
        if (error.name === "QuotaExceededError")
            return "The prompt exceeds the model's context capacity. Try a shorter prompt or fewer/smaller images.";
        if (error.name === "NotSupportedError")
            return "This browser or device does not support the requested text or image input. Check Chrome's Prompt API requirements.";
        if (error.name === "TimeoutError")
            return "Generation took too long. Try a shorter prompt.";
        return `${error.message || "The browser model failed."} Try again, or reload the page.`;
    }
    async function initialize() {
        if (disposed || initializing || session || !imageDiscoveryComplete || !globalThis.isSecureContext || !model)
            return;
        initializing = true;
        updateControls();
        ...
        try {
            ...
            const pending = model.create({
                ...sessionOptions,
                ...sampling,
                ...(instructions?.trim() ? { initialPrompts: [{ role: 'system', content: instructions }] } : {}),
    ...
    async function prompt(text, images) {
        if (stateless)
            throw new Error('Incremental prompts require Conversation session mode.');
        if (disposed || !session)
            throw new Error("Initialize the browser model first.");
        if (generating)
            throw new Error("A reply is already being generated.");
        ...
        if (images && !attachments.has(images))
            throw new Error('These attachments do not belong to this session.');
        const files = images?.snapshot() ?? [];
        if (files.length && !supportsImages)
            throw new Error('Image attachments are unsupported in this session.');
        generating = true;
        for (const item of attachments)
            item.setBusy(true);
        ...
        try {
            const input = files.length
                ? [{ role: 'user', content: [{ type: 'text', value: text }, ...files.map(value => ({ type: 'image', value }))] }]
                : text;
    ...
    async function promptMessages(messages, requestId, responseConstraint, sink) {
        if (disposed || !session)
            throw new Error('Initialize the browser model first.');
        if (!stateless)
            throw new Error('Full message histories require Stateless session mode.');
        if (generating)
            throw new Error('A reply is already being generated.');
        ...
        const input = messages.some(m => m.parts != null) ? messages.map(m => {
            ...
            for (const part of m.parts) {
                if (!part || typeof part.value !== 'string' || !part.value.trim())
                    throw new Error('Invalid message part.');
                if (part.type === 'text')
                    content.push({ type: 'text', value: part.value });
                else if (part.type === 'image') {
                    const files = retained.get(part.value);
                    if (!supportsImages || !files)
                        throw new Error('Image reference expired or belongs to another session. Start a new conversation.');
                    for (const value of files)
                        content.push({ type: 'image', value });
                }
                else
        ...
        if (sink && responseConstraint != null)
            throw new Error('Streaming structured output is unsupported. Use completed responses.');
        if (typeof session.clone !== 'function')
            throw new Error('This browser does not support session cloning required by the agent adapter.');
        generating = true;
        ...
        try {
            ...
            if (sink) {
                if (typeof cloned.promptStreaming !== 'function')
                    throw new Error('This browser does not support streaming.');
            ...
            if (responseConstraint != null && !abort.signal.aborted && error instanceof DOMException) {
                if (error.name === 'NotSupportedError')
                    throw new Error(`Structured output is not supported for this schema: ${error.message}`);
                if (error.name === 'SyntaxError')
                    throw new Error(`The browser could not produce valid structured output: ${error.message}`);
            }
            throw new Error(errorMessage(error));
        }
        finally {
            ...
            if (cloned && (!disposed || cloned !== activeClone))
                cloned.destroy();
    ...
    function cancelPrompt(requestId) {
    ...
    const samplingDiscovery = discoverSampling().then(info => {
        ...
        samplingInfo = info;
        updateControls();
        if (topKInput && info.maxTopK !== null)
            topKInput.max = info.maxTopK.toString();
        if (temperatureInput && info.maxTemperature !== null)
            temperatureInput.max = info.maxTemperature.toString();
        if (samplingMessage)
            samplingMessage.textContent = info.support === 'Supported'
    ...
    async function getSamplingInfo() {
        if (disposed)
            throw new Error('The browser model has been disposed.');
    ...
    discoverAvailability().then(state => {
        ...
        if (disposed || initializing || session)
            return;
...
function emptySamplingInfo(support) {
...
async function streamBatches(stream, sink, signal) {
    ...
    try {
        ...
        while (true) {
            ...
            if (typeof value !== 'string')
                throw new Error('The browser returned a non-text streaming chunk.');
            let offset = 0;
            ...
            if (++chunks >= 32 && batch.length) {
                ...
                if (text)
                    await abortable(sink.invokeMethodAsync('ReceiveAsync', text), signal);
                batch = carry;
                chunks = 0;
            }
        }
        if (batch)
            await abortable(sink.invokeMethodAsync('ReceiveAsync', batch), signal);
        signal.throwIfAborted();
    }
    finally {
        if (!ended)
